...
router.post('/users', 
  ...
  [
    ...
    body('role').isIn(['OPERATOR_QC', 'ENGINEER', 'QUALITY_MANAGER', 'AUDIT', 'ADMIN']).withMessage('Invalid role'),
  ],
  validate,
  async (req, res) => {
    try {
...
router.put('/users/:id', 
  ...
  [
    ...
    body('role').optional().isIn(['OPERATOR_QC', 'ENGINEER', 'QUALITY_MANAGER', 'AUDIT', 'ADMIN']),
